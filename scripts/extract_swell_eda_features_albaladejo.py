#!/usr/bin/env python3
"""Extract Albaladejo-style fixed-window EDA features from SWELL Poly5 (.S00).

Output per subject: ``data/processed_swell_eda_albaladejo_w20_s5/PP{N}.npz``
with the same schema as the HRV extractor:
``X``, ``y``, ``condition_code``, ``block_num``, ``timestamp_sec``, ``is_neutral``.
...
from src.data.swell_eda_albaladejo import (
    COND_TO_BINARY,
    COND_TO_CODE,
    FEATURE_NAMES,
    N_FEATURES,
...
OUT_DIR = Path("data/processed_swell_eda_albaladejo_w20_s5")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="SWELL Albaladejo-style EDA feature extraction")
    ...
    parser.add_argument(
        ...
        type=int,
        default=7,
        help="MATLAB 1-based logical index for EDA/skin channel. Default matches SWELL TMSi layout.",
    )
