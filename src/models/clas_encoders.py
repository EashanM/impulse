"""Encoder heads for CLAS windows: linear, GRU/LSTM on time steps, and 1D CNN+GRU/LSTM."""
...
class CLASLstmEncoder(nn.Module):
    """LSTM directly on (B, L, C); embedding is final-layer last hidden state."""
...
class CLASCnnLstmEncoder(nn.Module):
    """1D CNN stack + LSTM; `encode` returns last LSTM hidden state."""
